                             ...
                             UpdateIntKeys = case IntegerKeys of
                                   no_reads ->
                                    ...
                                    _->
                                       ...
                                       lists:sublist(IntegerKeys, NumReads-NumUpdates+1, NumUpdates)
                              end,
...
run(update_only_txn, KeyGen, ValueGen, State=#state{pb_pid=Pid, worker_id=Id,
    ...
    case antidotec_pb:start_transaction(Pid, OldCommitTime, [{static, true}]) of
        {ok, {static, {TimeStamp, TxnProperties}}}->
            ...
            BObjs = multi_get_random_param_new(UpdateIntKeys, TypeDict, ValueGen(), undefined, SetSize),
            case create_update_operations(Pid, BObjs, {static, {TimeStamp, [{exclusive_locks, [Consistency]}]}}, SeqWrites) of
                ok->
...
%    set_size=SetSize,
   ...
   case antidotec_pb:start_transaction(Pid, OldCommitTime, [{static, true}]) of
     ...
     %   true ->
           IntegerKeys = generate_keys(NumReads, KeyGen),
            BoundObjects = [{list_to_binary(integer_to_list(K)), get_key_type(K, TypeDict), ?BUCKET} || K <- IntegerKeys],
%            BObjs = multi_get_random_param_new(IntegerKeys, TypeDict, ValueGen(), undefined, SetSize),
%	    {BoundObjects, _Opp, _Param} = BObjs,
            case create_read_operations(Pid, BoundObjects, {static, {TimeStamp, TxnProperties}}, SeqReads) of
                {ok, RS} ->
                    case antidotec_pb:commit_transaction(Pid, {static, {TimeStamp, TxnProperties}}) of
                    {ok, BCommitTime}->
                          %{RS, IntegerKeys};
...
run(update_causal, KeyGen, ValueGen, State) ->
    run(update_only_txn, KeyGen, ValueGen, State#state{consistency = <<"causal">>, num_updates=1});
run(update_strong, KeyGen, ValueGen, State) ->
    run(update_only_txn, KeyGen, ValueGen, State#state{consistency = <<"strong">>, num_updates=1});
%% @doc the read command will run a transaction with a single read, and no updates.
run(read, KeyGen, ValueGen, State) ->
    run(read_only_txn, KeyGen, ValueGen, State#state{num_reads=1}).
